...
export default function SandboxPage() {
  const [serverUrl, setServerUrl] = useState('https://calculator.us-east.true-mcp.com/mcp');
  ...
  const NUMERIC_TYPES = ['integer', 'number', 'int', 'float', 'u8', 'u16', 'u32', 'u64', 'i8', 'i16', 'i32', 'i64', 'f32', 'f64'];
  const INTEGER_TYPES = ['integer', 'int', 'u8', 'u16', 'u32', 'u64', 'i8', 'i16', 'i32', 'i64'];
  const BOOLEAN_TYPES = ['boolean', 'bool'];

  const normalizeTypeToArray = (type: string | string[] | undefined): string[] => {
    if (!type) return [];
  ...
  const isNumericSchemaType = (type: string | string[] | undefined): boolean => {
    return normalizeTypeToArray(type).some(t => NUMERIC_TYPES.includes(t));
  };

  const isIntegerSchemaType = (type: string | string[] | undefined): boolean => {
    return normalizeTypeToArray(type).some(t => INTEGER_TYPES.includes(t));
  };

  const isBooleanSchemaType = (type: string | string[] | undefined): boolean => {
  ...
  const getPromptFormFields = (promptName: string): FormField[] => {
    ...
    return (prompt.arguments || []).map((arg: any) => ({
      name: arg.name,
      label: arg.name.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase()),
      type: 'string',
      ...
      if (args[1] && typeof args[1] === 'object' && args[1].body) {
        ...
        if (typeof bodySnippet === 'string') {
          try {
            ...
            addLog(`-> [OUTGOING HTTP POST]: ${JSON.stringify(parsed)}`, 'system');
          } catch (e) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      console.error(err);
      setStatus('error');
      setErrorMsg(err.message || 'Failed to connect. Ensure the URL is a valid MCP endpoint supporting HTTP POST.');
  ...
  const fetchData = async (mcpClient: PmcpWasmClient) => {
    try {
      ...
      const fetchedTools = await mcpClient.list_tools();
      setTools(fetchedTools || []);
      addLog(`Found ${fetchedTools?.length || 0} tools.`);
      
      addLog('Fetching available resources...');
      try {
        const fetchedResources = await mcpClient.list_resources();
        setResources(fetchedResources || []);
        addLog(`Found ${fetchedResources?.length || 0} resources.`);
      } catch(e) { addLog('Resources not supported by server.'); }
      
      addLog('Fetching available prompts...');
      try {
        const fetchedPrompts = await mcpClient.list_prompts();
        setPrompts(fetchedPrompts || []);
        addLog(`Found ${fetchedPrompts?.length || 0} prompts.`);
      } catch(e) { addLog('Prompts not supported by server.'); }
  ...
  const handleCallTool = async () => {
    ...
    try {
        ...
        if (schema) {
          if (isNumericSchemaType(schemaType)) {
            ...
            if (!isNaN(numValue) && value !== '' && value !== undefined) {
              coercedArgs[key] = isIntegerSchemaType(schemaType) ? Math.floor(numValue) : numValue;
            }
            ...
            if (typeof value === 'string') {
              try {
                ...
                if (schemaType === 'array') {
                  coercedArgs[key] = value.split(',').map((s: string) => s.trim()).filter(Boolean);
                } else {
      ...
      const result = await client.call_tool(selectedTool, coercedArgs);
  ...
  const handleReadResource = async () => {
    ...
    try {
      const result = await client.read_resource(selectedResource);
  ...
  const handleGetPrompt = async () => {
    ...
    try {
      const result = await client.get_prompt(selectedPrompt, promptFormData);
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="border-b border-white/10 pb-6">
          ...
          <p className="text-gray-400 mt-2 text-lg">
            Powered by the <span className="text-white font-semibold">Rust PMCP WASM Client</span>. Connect directly to any stateless HTTP MCP server from your browser.
          </p>
        </div>
        ...
        <div className="glass-card p-6 border-l-4 border-l-lcars-blue flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-grow w-full">
            <div className="flex justify-between items-end mb-2">
              <label className="block text-sm font-mono text-gray-400">SERVER_URL</label>
              <select 
              ...
              >
                ...
                <option value="https://calculator.us-east.true-mcp.com/mcp">Calculator</option>
                <option value="https://london-tube.us-east.true-mcp.com/mcp">London Tube</option>
                <option value="https://reinvent.us-east.true-mcp.com/mcp">AWS re:Invent</option>
              </select>
            </div>
            <input 
              type="text" 
          ...
          <div className="flex gap-4 w-full md:w-auto">
            {status !== 'connected' ? (
              ...
              >
                {status === 'connecting' ? <div className="w-5 h-5 rounded-full border-2 border-black border-t-transparent animate-spin" /> : <Plug className="w-5 h-5" />}
                Connect
              </button>
            ) : (
              <button 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="glass-card min-h-[400px] p-6 relative">
              {status !== 'connected' && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-[2px] z-10 rounded-xl">
                  <span className="font-mono text-gray-500 border border-white/10 bg-black/50 px-4 py-2 rounded-md">Awaiting Connection...</span>
                </div>
              )}
              ...
              {activeTab === 'tools' && (
                <div className="grid grid-cols-1 md:grid-cols-5 gap-6 min-h-[400px]">
                  ...
                  <div className="md:col-span-2 border-r border-white/10 pr-6 space-y-2 max-h-[600px] overflow-y-auto">
                    <h3 className="font-mono text-xs text-gray-500 mb-4 tracking-wider">AVAILABLE_TOOLS</h3>
                    {tools.map((t, idx) => (
                      <button
                        key={idx}
                        onClick={() => { setSelectedTool(t.name); setToolFormData({}); setToolResult(''); }}
                      ...
                      >
                        <div className="font-mono font-semibold text-sm truncate">{t.name}</div>
                      </button>
                    ))}
                    {tools.length === 0 && status === 'connected' && (
                      <div className="text-gray-500 text-sm italic">No tools found.</div>
                    )}
                  </div>
                  ...
                  <div className="md:col-span-3 flex flex-col">
                    {selectedTool ? (
                      <>
                        <h3 className="font-bold text-lg mb-2 text-lcars-orange">{selectedTool}</h3>
                        <p className="text-sm text-gray-400 mb-4">{tools.find(t => t.name === selectedTool)?.description}</p>
                        
                        <div className="space-y-4 mb-4">
                          {getToolFormFields(selectedTool).map(field => (
                            <div key={field.name} className="bg-black/20 p-3 rounded-md border border-white/5">
                              ...
                              {field.type === 'boolean' ? (
                                <input
                                  type="checkbox"
                                  checked={toolFormData[field.name] ?? field.defaultValue ?? false}
                                  onChange={e => setToolFormData(prev => ({ ...prev, [field.name]: e.target.checked }))}
                                  className="w-4 h-4 accent-lcars-orange"
                                />
                              ) : field.type === 'number' || field.type === 'integer' ? (
                                <input
                                  type="number"
                                  value={toolFormData[field.name] ?? field.defaultValue ?? ''}
                                  onChange={e => setToolFormData(prev => ({ ...prev, [field.name]: e.target.valueAsNumber }))}
                                ...
                                <input
                                  type="text"
                                  value={toolFormData[field.name] ?? field.defaultValue ?? ''}
                                  onChange={e => setToolFormData(prev => ({ ...prev, [field.name]: e.target.value }))}
                                  className="w-full bg-black/50 border border-white/10 rounded-md px-3 py-2 font-mono text-sm text-white focus:border-lcars-orange focus:outline-none"
                                />
                              )}
                            </div>
                          ...
                          {getToolFormFields(selectedTool).length === 0 && (
                            <div className="text-xs text-gray-500 italic mb-4">This tool takes no arguments.</div>
                          )}
                        </div>
                        ...
                        <label className="text-xs font-mono text-gray-500 mb-1">RESULT</label>
                        <div className="flex-grow bg-black/80 border border-white/10 rounded-md p-3 overflow-y-auto font-mono text-sm">
                          {toolResult ? (
                            <pre className="text-gray-300 whitespace-pre-wrap">{toolResult}</pre>
                          ) : (
              ...
              {activeTab === 'resources' && (
                <div className="grid grid-cols-1 md:grid-cols-5 gap-6 min-h-[400px]">
                  <div className="md:col-span-2 border-r border-white/10 pr-6 space-y-2 max-h-[600px] overflow-y-auto">
                    <h3 className="font-mono text-xs text-gray-500 mb-4 tracking-wider">AVAILABLE_RESOURCES</h3>
                    {resources.map((r, idx) => (
                      <button
                        key={idx}
                        onClick={() => { setSelectedResource(r.uri); setResourceResult(''); }}
                      ...
                      >
                        <div className="font-mono font-semibold text-sm truncate">{r.name}</div>
                        <div className="text-xs text-gray-500 truncate">{r.uri}</div>
                      </button>
                    ))}
                    {resources.length === 0 && status === 'connected' && (
                      <div className="text-gray-500 text-sm italic">No resources found.</div>
                    )}
                  </div>
                  <div className="md:col-span-3 flex flex-col">
                    {selectedResource ? (
                      <>
                        <h3 className="font-bold text-lg mb-2 text-lcars-orange">{resources.find(r => r.uri === selectedResource)?.name}</h3>
                        <p className="text-sm text-gray-400 mb-4">{resources.find(r => r.uri === selectedResource)?.description}</p>
                        <button 
                        ...
                        <label className="text-xs font-mono text-gray-500 mb-1">CONTENT</label>
                        <div className="flex-grow bg-black/80 border border-white/10 rounded-md p-3 overflow-y-auto font-mono text-sm">
                          {resourceResult ? (
                            <pre className="text-gray-300 whitespace-pre-wrap">{resourceResult}</pre>
                          ) : (
                            <span className="text-gray-600 italic">Waiting for execution...</span>
                          )}
                        </div>
                      ...
                      <div className="flex items-center justify-center h-full text-gray-500 italic">Select a resource to read</div>
                    )}
                  </div>
                </div>
              )}
              ...
              {activeTab === 'prompts' && (
                <div className="grid grid-cols-1 md:grid-cols-5 gap-6 min-h-[400px]">
                  <div className="md:col-span-2 border-r border-white/10 pr-6 space-y-2 max-h-[600px] overflow-y-auto">
                    <h3 className="font-mono text-xs text-gray-500 mb-4 tracking-wider">AVAILABLE_PROMPTS</h3>
                    {prompts.map((p, idx) => (
                      <button
                        key={idx}
                        onClick={() => { setSelectedPrompt(p.name); setPromptFormData({}); setPromptResult(''); }}
                      ...
                      >
                        <div className="font-mono font-semibold text-sm truncate">{p.name}</div>
                      </button>
                    ))}
                    {prompts.length === 0 && status === 'connected' && (
                      <div className="text-gray-500 text-sm italic">No prompts found.</div>
                    )}
                  </div>
                  <div className="md:col-span-3 flex flex-col">
                    {selectedPrompt ? (
                      <>
                        <h3 className="font-bold text-lg mb-2 text-lcars-orange">{selectedPrompt}</h3>
                        <p className="text-sm text-gray-400 mb-4">{prompts.find(p => p.name === selectedPrompt)?.description}</p>
                        ...
                        <div className="space-y-4 mb-4">
                          {getPromptFormFields(selectedPrompt).map(field => (
                            <div key={field.name} className="bg-black/20 p-3 rounded-md border border-white/5">
                              ...
                              <input
                                ...
                                onChange={e => setPromptFormData(prev => ({ ...prev, [field.name]: e.target.value }))}
                          ...
                          {getPromptFormFields(selectedPrompt).length === 0 && (
                            <div className="text-xs text-gray-500 italic mb-4">This prompt takes no arguments.</div>
                          )}
                        </div>
                        ...
                        <label className="text-xs font-mono text-gray-500 mb-1">MESSAGES</label>
                        <div className="flex-grow bg-black/80 border border-white/10 rounded-md p-3 overflow-y-auto font-mono text-sm">
                          {promptResult ? (
                            <pre className="text-gray-300 whitespace-pre-wrap">{promptResult}</pre>
                          ) : (
                            <span className="text-gray-600 italic">Waiting for execution...</span>
                          )}
                        </div>
          ...
          <div className="glass-card flex flex-col h-[calc(100vh-8rem)] sticky top-24">
            <div className="border-b border-white/10 p-4 flex items-center gap-2">
              ...
              <span className="font-mono text-sm font-semibold tracking-wider">SYSTEM_LOGS</span>
            </div>
            <div className="flex-grow p-4 overflow-y-auto font-mono text-xs space-y-2 bg-black/30">
              {logs.map((log, idx) => (
                <div key={idx} className={`${
                  log.includes('[ERROR]') ? 'text-red-400' : 
                  log.includes('[SYSTEM]') ? 'text-lcars-blue' : 'text-gray-400'
                }`}>
                  {log}
                </div>
              ))}
